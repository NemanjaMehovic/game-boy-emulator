...
uint8 Timer::read_tima() const {
    return tima;
}

void Timer::write_tima(uint8 val) {
...
uint8 Timer::read_tma() const {
    return tma;
}

void Timer::write_tma(uint8 val) {
    tma = val;
    if (state == State::Reload) {
        tima = tma;
    }
...
uint8 Timer::read_tac() const {
    return tac | 0xF8;
}

void Timer::write_tac(uint8 val) {
    tac = val & 0x07;
    falling_edge();
}
...
void Timer::write(uint16 addr, uint8 val) {
    switch (addr) {
        case 0xFF04:
            reset_div();
            break;
...
uint8 Timer::read(uint16 addr) const {
    switch (addr) {
        case 0xFF04:
            return read_div();
        case 0xFF05:
